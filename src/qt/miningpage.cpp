    ...
    if (!outputString.isEmpty())
    {
        ...
        for (i=0; i<list.size(); i++)
        {
            ...
            if (line.contains("accepted:"))
                reportToList("Share accepted! Woo Hoo! Feel The LiZi!!!", SHARE_SUCCESS, getTime(line));
                ...
                reportToList("New block detected on the LiZi network. Beginning work on the new block.", LONGPOLL, getTime(line));
...
void MiningPage::minerError(QProcess::ProcessError error)
{
    ...
    {
        reportToList("Miner failed to start. Make sure you have the minerd executable and any associated files in the same directory as LiZi-qt.", ERROR, NULL);
    }
}
