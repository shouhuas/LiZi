...
const string strMessageMagic = "LiZi Signed Message:\n";
...
bool CTxMemPool::addUnchecked(const uint256& hash, CTransaction &tx)
{
	...
	CTransactionDB trxDB("wc");
	trxDB.WriteTx(hash, tx);
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if(nHeight == 1) {
		nSubsidy = 100000000 * COIN; // LiZi instamine. 5% of 200 mil = 10 mil
    } 
else
	  {
		nSubsidy = 1000 * COIN; // LiZi instamine. 5% of 200 mil = 10 mil
    } 
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
    ...
    int64 nSubsidy = nCoinAge * nRewardCoinYear / 365;  // LiZi no / COIN so PoS is correct
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in LiZi
    bool fStrictPayToScriptHash = true; // Always active in LiZi
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "LiZi", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "I Love You LiZi";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("4b01bf0907e6cf5ef92d65e569cc058a9e31638f0baa076e9dadd9b1fec65571"));
    ...
    // ppcoin: if checkpoint master key changed must reset sync-checkpoint
    {
		...
		CTransactionDB tr("rc");
		tr.ReadAll(mempool.mapTx);
    }
